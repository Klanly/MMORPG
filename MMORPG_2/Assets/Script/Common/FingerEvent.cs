...
if (Input.touchCount > 1)
	...
	void OnFingerUp( int fingerIndex, Vector2 fingerPos, float timeHeldDown )
	{
        if (m_PrevFinger == 1)
        {
            ...
            //如果UGUI UI摄像机存在
            if (UI_Camera.Instance.Camera != null)
            {
                Ray rayUI = UI_Camera.Instance.Camera.ScreenPointToRay(Input.mousePosition);
                if (Physics.Raycast(rayUI, Mathf.Infinity, 1 << LayerMask.NameToLayer("UI")))
                {
                    return;
                }
    ...
    {
        if(WindowUtil.Instance.OpenWindowCount>0) return;
        m_PrevFinger = 3;
