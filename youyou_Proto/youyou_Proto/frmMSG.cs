...
namespace ReadExcel
{
    public partial class frmMSG : Form
    {
        ...
        //private string path = @"F:\YouYouTools\youyou_Proto\CreateProto";
        //客户端路径
        private string clientPath = @"E:\Project\Unity3D 5.6.2f1\MMORPG_2\Assets\Script\Data\Proto\CreateProto";
        private string clientPathLua = @"E:\Project\Unity3D 5.6.2f1\MMORPG_2\Assets\Script\Data\Proto\CreateLuaProto";
        //服务端路径
        private string serverPath = @"E:\Project\GameServer\GameServerApp\GameServerApp\Proto\CreateProto";
        private string serverPathLua = @"E:\Project\GameServer\GameServerApp\GameServerApp\Proto\CreateLuaProto";
        ...
        {
            CreateProto(m_CurrentProto,clientPath,clientPathLua, true);
        }
        ...
        private void CreateProto(MyProto proto,string path,string pathLua, bool isSave = false)
        {
            if (isSave)
            {
                //生成协议代码
            ...
            sbr.Append("\r\n");
            sbr.AppendFormat("/// <summary> {0} </summary>\r\n", proto.ProtoCnName);
            ...
            foreach (var item in lst)
            {
                if (!string.IsNullOrEmpty(item.AttToLoop))
                {
                    //这里是循环项的从属属性
                    sbr.AppendFormat("    /// <summary> {0} </summary>\r\n", item.AttCnName);
                    sbr.AppendFormat("    public List<{0}> {1}List;\r\n", item.AttType, item.AttEnName);
                }
                else
                {
                    //生成标准的基本属性
        ...
        private void CreateProtoCodeDef(string path,string pathLua)
        {
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
        ...
        {
            CreateProtoCodeDef(clientPath,clientPathLua);

            foreach (var menu in DataMgr.Instance.MenuList)
            {
                foreach (var proto in menu.ProtoList)
                {
                    this.CreateProto(proto,clientPath,clientPathLua);
                }
            }
        ...
        private void txtProtoCnName_TextChanged(object sender, EventArgs e)
        {
        ...
        {
            CreateProtoCodeDef(serverPath, serverPathLua);

            foreach (var menu in DataMgr.Instance.MenuList)
            {
                foreach (var proto in menu.ProtoList)
                {
                    this.CreateProto(proto, serverPath, serverPathLua);
                }
            }
        ...
        private void txtProtoDesc_TextChanged(object sender, EventArgs e)
        {
        ...
        {
            CreateProto(m_CurrentProto, serverPath, serverPathLua, true);
        }
    }
}