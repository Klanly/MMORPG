--角色技能主动通知协议
RoleData_SkillResponseProto = { ProtoCode = 31001, SkillCount = 0, CurrSkillDataTable = { } }

--这句是重定义元表的索引，就是说有了这句，这个才是一个类
RoleData_SkillResponseProto.__index = RoleData_SkillResponseProto;

function RoleData_SkillResponseProto.New()
    local self = { }; --初始化self
    setmetatable(self, RoleData_SkillResponseProto); --将self的元表设定为Class
    return self;
end
...
--发送协议
function RoleData_SkillResponseProto.SendProto(proto)
...
--解析协议
function RoleData_SkillResponseProto.GetProto(buffer)

    local proto = RoleData_SkillResponseProto.New(); --实例化一个协议对象
