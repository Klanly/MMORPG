--客户端发送战斗胜利协议
GameLevelSuccessRequestProto = { ProtoCode = 10402, GameLevelId = 0, Grade = 0, Star = 0, Exp = 0, Gold = 0, KillTotalMonsterCount = 0, KillMonsterTable = { }, GoodsTotalCount = 0, GetGoodsTable = { }, ChapterId = 0 }

--这句是重定义元表的索引，就是说有了这句，这个才是一个类
GameLevelSuccessRequestProto.__index = GameLevelSuccessRequestProto;

function GameLevelSuccessRequestProto.New()
    local self = { }; --初始化self
    setmetatable(self, GameLevelSuccessRequestProto); --将self的元表设定为Class
    return self;
end
...
--发送协议
function GameLevelSuccessRequestProto.SendProto(proto)
...
--解析协议
function GameLevelSuccessRequestProto.GetProto(buffer)

    local proto = GameLevelSuccessRequestProto.New(); --实例化一个协议对象
